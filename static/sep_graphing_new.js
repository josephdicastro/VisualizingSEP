...
//listen for changes in searchType Menu, and update menu accordingly
...
function loadSearchMenu(searchType) {
    d3.json('static/sep_network.json', function(data) {
        ...
        menu_nodes = data.nodes

        menu_nodes.forEach(function(node){
            if(node.entry_type==='thinker') {
                philosophers.push(node.title);
            }   else{
                ideas.push(node.title);
            }
        })
...
function build_sep_graph(searchTerm) {
    d3.json('static/sep_network.json', function(data) {
        ...
        //create secondary svg group so that we can add our graph elements together
        let g = g1.append('g')
