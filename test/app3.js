...
// let labelgroup = container.append("g").data(data.nodes)
// let label =
//     .attr("class", "labels")
//     .selectAll("text")
...
//     .exit().remove();
        ...
        node.on("mouseover", focus)
            ...
            let node_title = data.nodes[0].title;
            let node_url = data.nodes[0].id;
            let node_entrytype = data.nodes[0].entry_type;

            let sidebar = d3.select("#sidebar")    
                sidebar.select("h3").text(node_title)
                sidebar.select("p").html(`Read the full article at: <a href="${node_url}">${node_url}</a>`)
